...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Upload video file
    $videoDestination = uploadFile('videoFile', '../../EducationContent/video/');
    if (!$videoDestination) {
        $_SESSION['errorMsg'] = "Failed to upload video.";
    }

    // Upload thumbnail
    $thumbnailDestination = uploadFile('thumbnailFile', '../../EducationContent/video/thumbnail/');
    if (!$thumbnailDestination) {
        $_SESSION['errorMsg'] = "Failed to upload thumbnail.";
    }

    // Check if both files uploaded successfully
    if (!$videoDestination || !$thumbnailDestination) {
        ...
        if ($resultCheck->num_rows > 0) {
            ...
            $updateQuery = "UPDATE educationcontent SET video = ?, thumbnail= ?, video_title = ?, video_description = ? WHERE chapter_id = ? AND topic_id = ?";
            $stmtUpdate = $conn->prepare($updateQuery);
            $stmtUpdate->bind_param("ssssii", $videoDestination, $thumbnailDestination, $videoTitle, $videoDescription, $chapterId, $topicId);
            if ($stmtUpdate->execute()) {
                $_SESSION['successMsg'] = "Existing row updated successfully.<br>";
            } else {
                $_SESSION['errorMsg'] = "Error updating existing row.<br>";
            }
        } else {
            // Row does not exist, insert a new row
            $insertQuery = "INSERT INTO educationcontent (course_id, chapter_id, topic_id, video, thumbnail, video_title, video_description) VALUES (?, ?, ?, ?, ?, ?, ?)";
            $stmtInsert = $conn->prepare($insertQuery);
            $stmtInsert->bind_param("iiissss", $courseId, $chapterId, $topicId, $videoDestination, $thumbnailDestination, $videoTitle, $videoDescription);
